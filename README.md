...
- Automatic handling of reserved/blocked UDP ports on Windows

Example:

`python udp_test.py --host-ip 192.168.15.15 --dut-ip 192.168.15.14 --dut-port 11451`

## Verilator Simulation

This repository also includes a Verilator + TAP virtual NIC simulation under `simulation/`.

Note: this Verilator + TAP simulation flow is Linux-only.
...
注意：该 Verilator + TAP 仿真流程仅支持 Linux。
...
- UDP 行列テスト + ランダムソークテスト（遅延統計付き）
- Windows の予約/禁止 UDP ポートを自動回避
...
注記: Verilator + TAP シミュレーションフローは Linux 専用です。
