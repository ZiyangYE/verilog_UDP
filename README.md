...
- Run interaction test (one-time sudo auth): `cd simulation && python3 test_interact.py`

The simulation bridges RMII traffic to a Linux TAP interface and can be used for ARP/UDP functional verification before FPGA deployment.
...
- 运行交互测试（一次 sudo 授权）：`cd simulation && python3 test_interact.py`

该仿真会把 RMII 流量桥接到 Linux TAP 接口，可在上板前完成 ARP/UDP 功能验证。
...
- 連携テスト実行（sudo 一回認証）: `cd simulation && python3 test_interact.py`

このシミュレーションは RMII トラフィックを Linux TAP インターフェースにブリッジし、FPGA 実機投入前の ARP/UDP 機能検証に利用できます。
